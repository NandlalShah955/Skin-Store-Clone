import {HiOutlineShoppingBag} from 'react-icons/hi'
import '../HomePage/HomePage.css'
import '../SkinCare/SkinCare.css'

function Tools() {
  return (
    <>
<h1 className="Shophai"> Shop Tools</h1>

<div className="Shopcateg">
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/53/Page-001-024853.png"
            alt=""
          />
        </div>
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/09/Page-002-024909.png"
            alt=""
          />
        </div>
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/16/Page-003-024916.png"
            alt=""
          />
        </div>
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/25/Page-004-024925.png"
            alt=""
          />
        </div>
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/34/Page-005-024934.png"
            alt=""
          />
        </div>
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/41/Page-006-024941.png"
            alt=""
          />
        </div>
      </div>

      <div className='topdiv'>
        <img src="https://static.thcdn.com/images/xlarge/webp/widgets/121-us/56/SS-Batching-Christmas-Category_Banner-EG1180x450_5-082556.jpg" alt="" />
      </div>

      <div className="overblog" id='overblog1'>
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/51/original-0224-STDCRE-32153-SS-BME-Skinstore-March-2022-Photography-Batching-Shot_15-500x500-074151.jpg"
            alt=""
          />
          <h4>Up to 40% off TriPollar</h4>
          <p>
       
Slip is the world’s first silk pillowcase offered as a beauty product which provides immediateand dramatic results.
          </p>
          <button>SHOP NOW</button>
        </div>
        <div className="Tripoller">
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/23/original-original-Gentlemen%27s_Playground-025731-071323.jpg"
            alt=""
          />
          <h4>25% off Lelo with code SINGLES</h4>
          <p>
          LELO’s story is inseparably entwined with the changing sexual landscape.


          </p>
          <button>SHOP NOW</button>
        </div>
        <div className="Tripoller" id='nichekro'>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/36/Screenshot_2022-03-07_153134-071336.png"
            alt=""
          />
          <h4>
          
25% off Michael Todd Beauty with code SINGLES</h4>
          <p>
           

         

          
You can buy the full range ESPA skin care products online and enjoy the luxury of ESPA at home.
          </p>
          <button>SHOP NOW</button>
        </div>
      </div>

      <div className="overblog" id='overblog2'>
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/10/original-AntiAgingCollection-SkinStore-500x500-044510.jpg"
            alt=""
          />
          <h4>25% off Trophy Skin with code SINGLES</h4>
          <p>
           

         
         
Receive a Trophy Skin Light Enhancing Serum when you buy a tool from the brand.
          </p>
          <button>SHOP NOW</button>
        </div>
        <div className="Tripoller">
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/17/original-Collage_widget_BeautyToolsCollage-071117.jpg"
            alt=""
          />
          <h4>25% off Nursui Jamie with code SINGLES</h4>
          <p>
          Professional skin care brand founded in 1996 with a conscious,
          </p>
          <button>SHOP NOW</button>
        </div>
        <div className="Tripoller" id='nichekro'>
          <img
            src="https://static.thcdn.com/images/small/webp/widgets/121-us/37/Refa_Square-020155-071137.jpg"
            alt=""
          />
          <h4>
          25% off ReFa with code SINGLES</h4>
          <p>
           

         
Enjoy the ultimate Spa-Inspired experience at home with our natural products.
          </p>
          <button>SHOP NOW</button>
        </div>
      </div>

      <h1 className="Shophai"> Best Sellers</h1>

      <div className="buying_people">
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp//productimg/480/480/12358556-1974888087906815.jpg"
            alt=""
          />
          <p>TriPollar STOP X Device - Rose Gold</p>
          <p>$199.75</p>
          <div><HiOutlineShoppingBag/>SHOP NOW</div>
        </div>
        <div>
          <img
            src="https://static.thcdn.com/images/small/webp//productimg/480/480/12531850-4234764793695699.jpg"
            alt=""
          />
          <p>Fur Silk Scrub 6 fl. oz</p>
          <p>$119.00</p>
          <div><HiOutlineShoppingBag/>SHOP NOW</div>
        </div>
        <div className="Riley">
          <img
            src="https://static.thcdn.com/images/small/webp//productimg/480/480/13696729-1254956313287585.jpg"
            alt=""
          />
          <p>T3 AireLuxe Hair Dryer </p>
          <p>$199.00</p>
          <div><HiOutlineShoppingBag/>SHOP NOW</div>
        </div>
        <div className="Riley">
          <img
            src="https://static.thcdn.com/images/small/webp//productimg/480/480/11288898-2044866329672154.jpg"
            alt=""
          />
          <p>
          
          NuFACE Trinity + Trinity ELE Attachment Set</p>
          <p>$449.00</p>
          <div><HiOutlineShoppingBag/>SHOP NOW</div>
        </div>
      </div>



      <h1 className="Shophai">Over On the Blog</h1>



<div className='extradiv'>
  <div>
    <img src="https://static.thcdn.com/images/medium/webp/widgets/121-us/30/Kassir-FI_1582321427-091730.jpg" alt="lady" />
    <h4>Meet the Experts:Introdicing Dr.Shiela Kassir</h4>
    <p>Dr. Kassir is one of our esteemed beauty experts! See which skincare tools she recommends here at SkinStore!</p>

  </div>
  <div>
    <img src="https://static.thcdn.com/images/medium/webp/widgets/121-us/41/original-0520-STDCRE-13944-SS-MF-SkinStore_Summer_Commercial_Campaign_Photography_%28June%29_THG0033114_-_BATCHING-shot_18-580x384-044941.jpg" alt="Product" />
    <h4>Learn about NuFace</h4>
    <p>See what's the scoop on the skincare tech scene with NuFACE during our Founder Q+A!</p>

  </div>
</div>



    </>
  )
}

export default Tools